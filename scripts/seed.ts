/**
 * seed.ts —— 生产（PostgreSQL / Neon）seed 脚本（ESM，用 tsx 运行）。
 *
 * - 目标库：由 DATABASE_URL 指向的生产 PostgreSQL（Neon / RDS 等）。
 * - 客户端：标准 @prisma/client（canonical prisma/schema.prisma），
 *   先执行 `prisma migrate deploy` 建表后再运行本脚本。
 * - 数据来源：data/published/{formula,acupoint,herb}.json
 *   结构为 Subject -> ContentItem -> KnowledgePoint 三层。
 * - 全部按主键 id 用 upsert 写入，幂等可重复执行。
 * - 题目(QuestionTemplate)按 KP 类型补 free_recall / recognition，出题 registry 运行时生成。
 *
 * 运行：DATABASE_URL=... DIRECT_URL=... npm run db:seed
 */
...
async function main(): Promise<void> {
  ...
  // 2) 为每个出现过的 KP 类型补 free_recall / recognition 模板
  let tplCount = 0;
